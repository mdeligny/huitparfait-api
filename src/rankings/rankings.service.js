...
function calculateGroupRanking ({ userId, groupId, page, pageSize }) {
  const sqlQuery = sql`
      ...
      GROUP BY
          u.id, ugb.created_at
      ORDER BY
          total_score DESC, total_score DESC, nb_perfects DESC, nb_predictions DESC, member_since DESC, name
`;
...
function calculateGeneralRanking ({ userId, page, pageSize }) {
  const sqlQuery = sql`
      ...
      GROUP BY
          u.id
      ORDER BY
          total_score DESC, total_score DESC, nb_perfects DESC, nb_predictions DESC, u.created_at, u.name
`;
